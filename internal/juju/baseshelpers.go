...
// differencesOfBases returns a slice of bases representing all the values in the
// first slice that are not in the second.
func differenceOfBases(one, two []base.Base) []base.Base {
	result := make([]base.Base, 0)
	for _, value1 := range one {
		for _, value2 := range two {
			if !value1.IsCompatible(value2) {
